      ...
      if (distance == 0 or distance > 3):
        ...
        for num_second_check in value_row:
          if (k != 0):
            ...
            if (distance == 0 or distance > 3):
              print(f"value row copy on entry {value_row_copy}")
              value_row_copy.pop(i+1)
              print(f"value row copy after pop {value_row_copy}")

              # have to check is the next number can be removed and still be in range
              l = 0
              for num_third_check in value_row_copy:
                if (l != 0):
                  # get distance
                  distance = abs(value_row_copy[l-1] - value_row_copy[l])
                  if (distance == 0 or distance > 3):
                    is_in_range = False
                    break
